...
import type {Locale,ReleaseOverride} from '@/domain/models'
...
export function ReleaseGates(){
 const t=useTranslations();const locale=useLocale() as Locale;const {organizationId}=useWorkspace();const orgSystems=systems.filter(system=>system.organizationId===organizationId);const policy=releasePolicies.find(item=>item.organizationId===organizationId)!
 ...
 const submit=form.handleSubmit(values=>{
  ...
  }catch{setOverrideError(t('ui.overrideFail'))}
 })
 const blocked=gates.filter(gate=>effectiveReleaseDecision(gate,overrides,now)==='BLOCKED').length
 return <><PageHeading namespace="release" titleKey="title" subtitleKey="subtitle"/><section className="release-alert"><span><LockKeyhole size={20}/></span><div><b>{t('ui.releaseBlocked',{count:blocked})}</b><p>{t('ui.gatesRecomputed')}</p></div></section><section className="panel"><h2>{t('release.policy')}: {policy.id==='RP-HEALTH-1'?t('ui.healthcarePolicy'):policy.id==='RP-BANK-1'?t('ui.bankPolicy'):t('ui.consumerPolicy')}</h2>{gates.length?gates.map(gate=>{
  ...
  return <article className="release-row" key={gate.id}><span className="release-icon"><LockKeyhole size={17}/></span><span className="release-name"><b>{system.name}</b><small>{system.versions.find(version=>version.id===system.activeVersionId)?.version}</small></span><StatusPill status={active?'READY':gate.decision}/><span className="release-note">{override?`${active?t('ui.overrideActive'):t('ui.overrideExpired')} · ${override.owner} · ${override.expiresAt}`:gate.reasons.map(reason=>reason==='Regression threshold exceeded'?t('ui.reasonRegression'):reason==='Unresolved high-severity finding'?t('ui.reasonHigh'):reason==='Unresolved critical finding'?t('ui.reasonCritical'):reason==='Human review required'?t('ui.reasonHuman'):t('ui.reasonStale')).join('; ')||t('ui.noBlockingConditions')}</span><button className="btn outline" onClick={()=>{setOpenForm(gate.id);setOverrideError('')}} disabled={gate.decision!=='BLOCKED'||!allowed}>{t('release.override')}</button>{openForm===gate.id&&<form className="override-form" onSubmit={submit}><label>{t('ui.ownerLabel')}<input {...form.register('owner')}/></label><label>{t('ui.rationaleLabel')}<input {...form.register('rationale')}/>{form.formState.errors.owner&&<small>{t('ui.ownerMin')}</small>}{form.formState.errors.rationale&&<small>{t('ui.rationaleMin')}</small>}</label><label>{t('ui.expiryLabel')}<input type="date" {...form.register('expiresAt')}/>{form.formState.errors.expiresAt&&<small>{t('ui.expiryRequired')}</small>}</label>{!allowed&&<p role="alert">{t('ui.permissionDenied')}</p>}{overrideError&&<p role="alert">{overrideError}</p>}<button className="btn primary">{t('ui.recordOverride')}</button><button type="button" className="btn outline" onClick={()=>setOpenForm('')}>{t('common.close')}</button></form>}<div className="gate-reasons">{gate.reasons.map(reason=><p key={reason}><ShieldAlert size={14}/>{reason==='Regression threshold exceeded'?t('ui.reasonRegression'):reason==='Unresolved high-severity finding'?t('ui.reasonHigh'):reason==='Unresolved critical finding'?t('ui.reasonCritical'):reason==='Human review required'?t('ui.reasonHuman'):t('ui.reasonStale')}</p>)}{findingNow.filter(finding=>finding.systemId===gate.systemId).map(finding=><p key={finding.id}><Link href={`/findings?finding=${finding.id}` as never}>{demoCopy(locale,'finding',finding.id,finding.title)}</Link> · <StatusPill status={finding.status}/></p>)}</div></article>
