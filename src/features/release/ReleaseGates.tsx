...
export function ReleaseGates(){
 ...
 return <><PageHeading namespace="release" titleKey="title" subtitleKey="subtitle"/><section className="release-alert"><span><LockKeyhole size={20}/></span><div><b>{t('ui.releaseBlocked',{count:blocked})}</b><p>{t('ui.gatesRecomputed')}</p></div></section><section className="panel"><h2>{t('release.policy')}: {policy.name}</h2>{gates.length?gates.map(gate=>{
